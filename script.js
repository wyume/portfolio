...
(function () {
          ...
          if (cloudMap && typeof cloudMap === 'object' && !Array.isArray(cloudMap)) {
            Object.keys(cloudMap).forEach(function(localKey) {
              ...
              if (urls.length === 0 && existing.length > 0) {
                // 云端清空了。只清 Supabase Storage URL，base64 本地数据保留
                var allUrls = existing.every(function(item) {
                  return typeof item === 'string' && item.indexOf('supabase.co/storage') > -1;
                });
                if (allUrls) {
                  merged = [];
                  removed = true;
  ...
  /* ============================================================
     云文件存/取（绕过故障的 Storage，直接存 base64 到 content 表）
  ...
  function _cloudFileLoad(key, callback) {
    if (!window.DS || !window.DS.isOnline()) { callback(null); return; }
    window.DS.loadContent('_files_' + key).then(function(data) {
  ...
  var data = {
    ...
    arch: {
      ...
      desc: '以国反 App 为基座，采用分布式架构为省市公安构建标准化反诈方案。',
      sections: [
        { h: '方案背景', b: '国家反诈中心 App 面向全国通用反诈场景建设，由公安部统一管理运营、集中部署，各省市公安缺乏数据管理与功能拓展的自主权限，通用版本难以适配本地警务机制与反诈实战打法。本方案旨在解决地方公安在数据接入、流程适配、触达反馈等方面的落地痛点，实现通用能力与本地实战的结合。' },
        { h: '实际做法', b: '对比地方独立建设与部中心统一扩展的弊端，确定「部级基座 + 省级定制」的分布式架构，兼顾成本投入、响应速度、灵活性与数据自主权。实施层面，依托部地通讯链路构建数据传输通道，明确数据同步与安全传输规范，保障数据稳定流转。业务层面，以国反 App 为基座，在公安网与互联网双域拓展本地化反诈服务，形成「部—省—市—区—所」五级联动的协同作战体系。' },
        { h: '落地成果', b: '建成部地一体化数据通讯链路与协同机制，支撑地方公安特色反诈服务的快速拓展，已在多个省市公安部署应用，服务覆盖地方用户超 1 亿。' }
      ],
    ...
    summary: {
      ...
      desc: '作为国版链超级节点，落地版权交易系统，打通确权、存证、发行、交易全链路。',
      sections: [
        { h: '方案背景', b: '数字版权与数藏市场快速兴起，但确权、存证、溯源等基础能力缺位，多方权责不清，导致确权难、维权难、流通难。「国版链」解决了确权登记与规范治理的顶层问题，但作为治理型基础设施，不宜直接经营交易业务，需由超级节点（中数链）承接其能力并落地为可市场化运营的版权交易系统。' },
        { h: '实际做法', b: '制定「版权/IP方—发行方—中数链—国版链」四方业务交互模式，明确各角色权责边界与交互流程，形成覆盖确权、存证、发行、交易、流转的完整业务规则体系，基于该体系完成中数链版权交易系统与国版数创交易端的设计，落地空投、0元购、盲盒、合成、二交等数藏交易场景，并配套构建中数链浏览器，实现链上溯源与合约核验。' },
        { h: '落地成果', b: '主导设计的版权交易系统已实现全流程线上化运营并顺利接入元潮互娱、上元文创、任意门科技等多家文创平台，形成合规闭环的版权交易业务体系。' }
      ],
    ...
    xinsheng: {
      ...
      desc: '整合信用卡账务管理、持牌信贷与消费场景的一体化消费金融平台。',
      sections: [
        { h: '方案背景', b: '信用卡已成为个人消费金融的核心载体，持卡人对账单管理与资金安排的需求日益增长。本方案定位于信用卡账务管理与消费金融服务，通过账户体系与资金流转设计，为持卡人提供一体化的账单管理、合规信贷与消费服务，助力其高效管理资金、持续经营信用。' },
        { h: '实际做法', b: '基于合规与体验的权衡，确定「前端统一 + 后台隔离」的架构，构建「个人支付电子户」与「个人商户电子户」双账户体系，配合借记卡、信用卡形成多账户资金流转闭环，前端应用层由 App 统一承载账务管理、消费信贷与消费场景，后端数据层将持牌信贷业务与平台服务分离。' },
        { h: '设计要点', b: '持牌信贷业务与平台服务在数据层彻底分离，满足持牌经营要求；征信数据源均通过持牌主体合规接入，并配套明确的用户授权流程，保障个人信息合法合规使用；账务管理、信贷、消费的功能边界清晰。' }
      ],
  ...
  var productData = {
    ...
    xinshengdai: {
      ...
      metrics: [],
      desc: '面向信用卡用户，提供多卡账单管理与还款规划等账务服务。以「前端统一 + 后台隔离」的架构，整合持牌信贷与消费场景，实现业务隔离，用户无需跨应用即可完成从账单管理到还款的全流程。',
      outputs: [],
      ...
      function renderSlnModal(currentFiles) {
        ...
        if (isCustom) {
          uploaded.forEach(function(f, idx) {
            var aType = slnType(f);
            var isDoc = aType === 'pdf' || aType === 'word' || aType === 'ppt';
            ...
            if (isDoc) {
              var dType = (f.origType === 'pdf' || f.origType === 'word' || f.origType === 'ppt') ? f.origType : aType;
              var iconSrc = dType === 'pdf' ? 'images/doc-type-pdf.png' : dType === 'word' ? 'images/doc-type-word.png' : 'images/doc-type-ppt.png';
              var label = slnName(f) || (dType === 'pdf' ? 'PDF 文件' : dType === 'word' ? 'Word 文件' : 'PPT 文件');
              var typeBtn = document.body.classList.contains('edit-locked') ? '<span class="sln-type-btn" style="position:relative;display:inline-flex;cursor:default"><img src="' + iconSrc + '" style="width:36px;height:36px;object-fit:contain" alt=""></span>' : '<span class="sln-type-btn" style="position:relative;display:inline-flex;cursor:pointer" onclick="event.stopPropagation();window._slnFileTypeCycle(\x27' + slnKey + '\x27,' + idx + ')" title="点击切换文件类型图标"><img src="' + iconSrc + '" style="width:36px;height:36px;object-fit:contain" alt=""></span>';
              imgHtml += '<div onclick="event.stopPropagation();window._openSlnPreview(\x27' + slnKey + '\x27,' + idx + ')" style="width:160px;height:130px;border-radius:10px;cursor:pointer;background:linear-gradient(rgba(255,255,255,.5),rgba(255,255,255,.5)),url(images/sln-cover-bg.png) center/cover no-repeat;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,.5);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;transition:all .25s ease" onmouseenter="this.style.transform=\x27translateY(-5px)\x27" onmouseleave="this.style.transform=\x27translateY(0)\x27">' + typeBtn + '<span style="font-size:10px;color:var(--text2);max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + label + '</span></div>';
            } else {
  ...
  function startEdit(el) {
    ...
    if (!isH4) {
      ...
      DOC_TYPES.forEach(function(t, ti) {
        ...
        btn.textContent = t.key === 'img' ? 'i' : (t.key === 'pdf' ? 'F' : t.key.charAt(0).toUpperCase());
        btn.type = 'button';
...
document.querySelector('.modal-box').classList.add('glass');
  ...
  document.querySelectorAll('#design .doc-card').forEach(function (card) {
    ...
    preview.dataset.bgUrl = imgUrl;
    card.appendChild(preview);
  });
  // Lazy load hover-preview background images when card is near viewport
  var previewBgObs = new IntersectionObserver(function(entries) {
    entries.forEach(function(entry) {
      if (entry.isIntersecting) {
        var el = entry.target;
        var url = el.dataset.bgUrl;
        if (url) {
          el.style.backgroundImage = 'url(' + url + ')';
          delete el.dataset.bgUrl;
        }
        previewBgObs.unobserve(el);
      }
    });
  }, { rootMargin: '300px' });
  document.querySelectorAll('.hover-preview').forEach(function(p) {
    previewBgObs.observe(p);
  });
  ...
  function _docLoad(k,cb){var done=false;function resolve(a){if(!done){done=true;cb(a);}}var cached=window._docImgs[k];if(cached&&cached.length)return resolve(cached);var r=localStorage.getItem(k);if(r){try{var a=JSON.parse(r);if(!(Array.isArray(a)&&a.length===1&&a[0]==='__IDB__')){if(a.length){window._docImgs[k]=a;return resolve(a);}}}catch(e){}}_prodDBLoad(k,function(db){var a=(db&&db.length)?db:[];if(a.length){window._docImgs[k]=a;resolve(a);return;}_cloudFileLoad(k,function(cd){if(cd){window._docImgs[k]=cd;try{localStorage.setItem(k,JSON.stringify(cd));}catch(e){}_prodDBSave(k,cd);resolve(cd);}else{resolve([]);}});});setTimeout(function(){resolve([]);},2000);}
  ...
  function _buildDocLightbox(k,itemName,files){
    ...
    function showFile(fi){idx=fi;updateUI();
      ...
      stage.innerHTML='';
      if(ft==='image'){var img=document.createElement('img');img.loading='lazy';img.decoding='async';img.src=fd;img.className='doc-lb-img';stage.appendChild(img);}
  ...
  function _prodDBLoad(k,cb){if(_prodDB){var t=_prodDB.transaction('imgs','readonly');var r=t.objectStore('imgs').get(k);r.onsuccess=function(){var a=r.result||[];if(a.length){cb(a);return;}_cloudFileLoad(k,function(cd){if(cd){try{var tw=_prodDB.transaction('imgs','readwrite');tw.objectStore('imgs').put(cd,k);}catch(e){}cb(cd);}else{cb([]);}});};r.onerror=function(){_cloudFileLoad(k,function(cd){cb(cd||[]);});};}else{_prodQueue.push(function(){_prodDBLoad(k,cb);});}}
  ...
  function _buildSlnLightbox(k,files,startIdx){
    ...
    function showFile2(fi){idx=fi;updateUI2();
      ...
      var f=files[fi];var ft=(typeof f==='string')?'image':(f.type||'image');var fd=(typeof f==='string')?f:f.data;var fn=(typeof f==='string')?'':(f.name||'');
      stage2.innerHTML='';
      if(ft==='image'){var img=document.createElement('img');img.loading='lazy';img.decoding='async';img.src=fd;img.className='doc-lb-img';stage2.appendChild(img);}
  ...
  var _di=[{p:'百威 ABI',t:'Mobile / TV',im:['images/budweiser-logo.png']},{p:'永辉超市',t:'PC',im:['images/yonghui-logo.png']},{p:'达能水业',t:'Mobile',im:['images/danone-logo.png']},{p:'日本罗森',t:'PC',im:['images/lawson-logo.png']},{p:'美的',t:'M-Box',im:['images/美的.png']}];
  ...
  window._dd=function(idx){var k='design_img_'+idx;localStorage.removeItem(k);if(_prodDB){var t=_prodDB.transaction('imgs','readwrite');t.objectStore('imgs').delete(k);}_syncCloudUrlsAfterDelete(k);_cloudFileSave(k,[]);var c=document.querySelectorAll('.design-gallery-item')[idx];if(c)_refreshDesignCard(c,idx,k,[]);};
